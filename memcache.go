...
func NewChClient(servers []string, weights []uint32) *Client {
	if weights == nil {
		weights = make([]uint32, 0)
	}
	for len(weights) < len(servers) {
		weights = append(weights, 0)
	}
	chs := new(ChServerList)
	weightedServers := make([]*WeightedServer, len(servers))
	for i, s := range servers {
		weightedServers[i] = &WeightedServer{s, weights[i]}
	}
	chs.SetServers(weightedServers...)
	return NewFromSelector(chs)
}
